        ...
        with open(options.config_file, 'r') as f:
            for line in f:
                ...
                if not Path(curr_sam).exists():
                    raise ValueError(
                        "SAM file '%s' does not exist!" % curr_sam)
                ...
                if dataname in existing_datasets:
                    ...
                    if not curr_sam.endswith(".sam"):
                        raise ValueError(
                            'Last field in config file must be a .sam file')
...
def main():
    ...
    with mp.Pool(processes=threads) as pool:
        run_info = init_run_info(
            database, build, min_coverage, min_identity, tmp_dir=options.tmp_dir)
        run_info.outfiles = init_outfiles(
            options.outprefix, tmp_dir=options.tmp_dir)
